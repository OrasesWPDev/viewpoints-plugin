...
class Viewpoints_ACF_Manager {
	...
	public function import_field_groups() {
		...
		if (!file_exists($json_file)) {
			...
			// If file doesn't exist, create it from the hardcoded field group
			viewpoints_plugin_log('Viewpoints ACF Manager: Creating field group JSON file');
			$this->create_default_field_group_json();
			
			if (!file_exists($json_file)) {
				viewpoints_plugin_log('Viewpoints ACF Manager: Failed to create field group JSON file');
				return;
			}
		}
	...
	/**
	 * Create default field group JSON file if it doesn't exist
	 *
	 * @since 1.0.0
	 */
	private function create_default_field_group_json() {
		$field_group = array(
			"key" => "group_viewpoints_fields",
			"title" => "Viewpoint Field Groups",
			"fields" => array(
				array(
					"key" => "field_67d1e26112a1f",
					"label" => "Viewpoint Excerpt",
					"name" => "viewpoint_excerpt",
					"aria-label" => "",
					"type" => "wysiwyg",
					"instructions" => "used for archive listings - Use P (<P>) with block quote html",
					"required" => 0,
					"conditional_logic" => 0,
					"wrapper" => array(
						"width" => "",
		...
		$json_content = json_encode($field_group, JSON_PRETTY_PRINT);
		$json_file = $this->json_dir . $this->field_group_filename;
		
		viewpoints_plugin_log('Viewpoints ACF Manager: Writing field group JSON to file: ' . $json_file);
		$result = file_put_contents($json_file, $json_content);
		
		if ($result === false) {
			viewpoints_plugin_log('Viewpoints ACF Manager: Failed to write field group JSON file');
		} else {
			viewpoints_plugin_log('Viewpoints ACF Manager: Successfully wrote ' . $result . ' bytes to field group JSON file');
		}
	}
